...
void OrynKernelModuleManifestInit(void)
{
    ...
    SetModule(OrynKernelModuleProcess, "Process", "kernel thread structure, user process structure, user thread structure, per-process address spaces, ownership, lifecycle state, copy-on-write child foundation", "BootInfoMemoryMap,BootInfoKernelRange", requires_OrynKernelModuleProcess, 1U, 1, 0, 1, "OrynKernelModuleDefaultStop", "OrynKernelModuleDefaultPanic", "OrynKernelModuleDefaultShutdown", OrynKernelModuleDefaultStop, OrynKernelModuleDefaultPanic, OrynKernelModuleDefaultShutdown);
    SetModule(OrynKernelModuleScheduler, "Scheduler", "kernel thread records, scheduler-ready stacks, guarded stack allocation, timer wheel, sleep queues, work queues, wait queues, per-CPU run queues, pre-emptive round-robin scheduler", "BootInfoMemoryMap,BootInfoKernelRange", requires_OrynKernelModuleScheduler, 2U, 1, 0, 1, "OrynKernelModuleDefaultStop", "OrynKernelModuleDefaultPanic", "OrynKernelModuleDefaultShutdown", OrynKernelModuleDefaultStop, OrynKernelModuleDefaultPanic, OrynKernelModuleDefaultShutdown);
}
