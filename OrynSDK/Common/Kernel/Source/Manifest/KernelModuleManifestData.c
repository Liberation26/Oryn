...
void OrynKernelModuleManifestInit(void)
{
    ...
    SetModule(OrynKernelModuleProcess, "Process", "process IDs, thread IDs, parent-child relationships, exit status, wait semantics, kernel thread structure, user process structure, user thread structure, per-process address spaces, ownership, lifecycle state, copy-on-write child foundation", "BootInfoMemoryMap,BootInfoKernelRange", requires_OrynKernelModuleProcess, 1U, 1, 0, 1, "OrynKernelModuleDefaultStop", "OrynKernelModuleDefaultPanic", "OrynKernelModuleDefaultShutdown", OrynKernelModuleDefaultStop, OrynKernelModuleDefaultPanic, OrynKernelModuleDefaultShutdown);
